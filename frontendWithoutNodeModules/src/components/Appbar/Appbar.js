...
import CartLogo from '../images/cartlogo/cartLogo';
import OrderLogo from '../images/orderlogo/orderLogo';
import MoreIcon from '@mui/icons-material/MoreVert';
import { Collapse, List, ListItemButton, ListItemIcon, ListItemText, Typography } from '@mui/material';
import ProfilePicture from '../images/profilePicture/profilePicture';
import { useAppDispatch, useAppSelector } from '../../redux/hooks';
import { getAllUsersByKey, getCurrentNr } from './actions';
...
const Appbar = () => {
  ...
  const renderMobileMenu = (
    ...
    >
      <MenuItem onClick={() => goToPage(paths.ORDERS)}>
      ...
      <MenuItem onClick={() => goToPage(paths.CART)}>
        <IconButton
          size="large"
  ...
  React.useEffect(() => {
    if( userAccount ) {
      dispatch(getCurrentNr(userAccount.data.returnValue.username)).then((e) => {
        setOrderNumber(e.payload.data.returnValue[0]);
        setCartNumber(e.payload.data.returnValue[1]);
    }); }
  }, [userAccount])
      ...
      <AppBar position="static" sx={{ height: '75px' }}>
        <Toolbar>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' } }}>
            ...
            <IconButton size="large" aria-label={`show ${orderNumber} new mails`} color="inherit" onClick={() => goToPage(paths.ORDERS)}>
              <Badge badgeContent={orderNumber} color="error">
                <OrderLogo />
              </Badge>
            </IconButton>
            <IconButton
              onClick={() => goToPage(paths.CART)}
              size="large"
              aria-label={`show ${cartNumber} new notifications`}
              color="inherit"
            >
              <Badge badgeContent={cartNumber} color="error">
                <CartLogo />
